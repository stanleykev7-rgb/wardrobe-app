...
def random_outfit_combo(closet: list, weather: dict, bias: int = 0, exclude_keys: set = None, max_attempts: int = 8) -> dict:
    """
    Picks ONE outfit combo per zone, randomized among the top few
    weather-appropriate candidates rather than always the single closest
    match — built for the swipe feature ("Tinder for outfits"; see
    DECISIONS.md ADR-016), where showing the same deterministic pick
    every time would defeat the point of swiping through options.
    Reuses candidates_for_zone()'s existing warmth/waterproof/in_laundry
    filtering so this stays consistent with every other picking path in
    this module rather than inventing a second filtering rule.

    exclude_keys: a set of (top_id, bottom_id, feet_id, head_id) tuples to
    avoid regenerating (e.g. combos already disliked twice — see
    swipe_store.get_disliked_combo_keys()). If every attempt within
    max_attempts collides with an excluded combo, returns the last combo
    tried anyway rather than looping forever on a very small closet —
    the swipe feed showing an already-disliked combo occasionally on a
    tiny closet is a much smaller problem than it hanging.

    Returns: {"picks": {zone: item_or_None}, "target_warmth": int}
    """
    ...
    for attempt in range(max_attempts):
        picks = {}
        for zone in ZONES_REQUIRED + ZONES_OPTIONAL:
            candidates = candidates_for_zone(closet, zone, target, need_waterproof, limit=4)
            picks[zone] = random.choice(candidates) if candidates else None
        key = tuple(picks[z]["id"] if picks[z] else None for z in ("top", "bottom", "feet", "head"))
